...
// ১. প্রপস হিসেবে searchTerm এবং setSearchTerm গ্রহণ করা হলো
export default function Header({ searchTerm, setSearchTerm }) {
  ...
  return (
    <nav className="flex items-center bg-[#131921] p-2 sticky top-0 z-50 justify-between gap-2 md:gap-4">
      ...
      {/* Search Bar - ২. ভ্যালু এবং অন-চেঞ্জ কানেক্ট করা হলো */}
      <div className="flex flex-1 items-center h-9 md:h-10 rounded-md bg-[#febd69] hover:bg-[#f3a847] cursor-pointer">
        <input
          ...
          value={searchTerm} // বর্তমান সার্চ টেক্সট
        ...
        <Link to={!user && "/login"}>
          ...
          >
            <p className="text-[10px] md:text-xs">
              Hello, {user ? (user.displayName || user.email.split('@')[0]) : "Guest"}
            </p>
